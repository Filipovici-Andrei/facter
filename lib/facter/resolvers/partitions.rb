    ...
    class Partitions < BaseResolver
      ...
      class << self
        ...
        def populate_from_blkid(partition_name, blkid_and_lsblk)
          return {} unless blkid_command?(blkid_and_lsblk)
        ...
        def blkid_command?(blkid_and_lsblk)
          return blkid_and_lsblk[:blkid_exists] unless blkid_and_lsblk[:blkid_exists].nil?

          output = Facter::Core::Execution.execute('which blkid', logger: log)

          blkid_and_lsblk[:blkid_exists] = !output.empty?
        end
        ...
        def populate_from_lsblk(partition_name, blkid_and_lsblk)
          return {} unless lsblk_command?(blkid_and_lsblk)
        ...
        def lsblk_command?(blkid_and_lsblk)
          return blkid_and_lsblk[:lsblk_exists] unless blkid_and_lsblk[:lsblk_exists].nil?

          output = Facter::Core::Execution.execute('which lsblk', logger: log)

          blkid_and_lsblk[:lsblk_exists] = !output.empty?
        end
